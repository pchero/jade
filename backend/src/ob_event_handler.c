...
int run_outbound(void)
{
  ...
  if(tmp_const == NULL) {
    ...
    slog(LOG_NOTICE, "Could not get correct event_time_fast value. Set default. event_time_fast[%s]\n", tmp_const);
  }
  slog(LOG_NOTICE, "Event delay time for fast event. event_time_fast[%s]\n", tmp_const);
...
//      );
  ...
  switch(dial_mode) {
    ...
    default: {
      slog(LOG_ERR, "No match dial_mode. dial_mode[%d]\n", dial_mode);
    }
    break;
  }
...
//    update_campaign_info_status(ast_json_string_get(ast_json_object_get(j_camp, "uuid")), E_CAMP_STOPPING);
  ...
  // dial to customer
  j_res = NULL;
  dial_type = json_integer_value(json_object_get(j_dial, "dial_type"));
  switch(dial_type) {
...
//      j_res = ami_cmd_originate_to_application(j_dial);
  ...
  tmp = json_dumps(j_res, 0);
  slog(LOG_DEBUG, "Check value. tmp[%s]\n", tmp);
  sfree(tmp);
  json_decref(j_res);
    ...
    )
{
  ...
  slog(LOG_DEBUG, "Available destination count. cnt[%d]\n", cnt_avail);
  ...
  slog(LOG_DEBUG, "Service level. level[%d]\n", plan_service_level);

  // get current dialing count
  cnt_current_dialing = rb_dialing_get_count_by_camp_uuid(json_string_value(json_object_get(j_camp, "uuid")));
  ...
  slog(LOG_DEBUG, "Current dialing count. count[%d]\n", cnt_current_dialing);
